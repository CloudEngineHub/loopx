...
const marker = /<script\b[^>]*data-loopx-analytics[^>]*><\/script>\s*/g;
let count = 0;
async function visit(dir) {
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    ...
    if (id && canonical?.startsWith("https://loopx-project.github.io/loopx/") &&
      ...
      html = html.replace("</head>", `<script defer data-loopx-analytics data-measurement-id="${id}" src="/loopx/site-assets/analytics.js"></script>\n</head>`);
      count++;
    }
