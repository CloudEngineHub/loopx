...
| Agent-lane status projection | `loopx.control_plane.status.agent_lane_projection.compact_agent_lane_status_payload_for_display` | Per-agent lane compaction/filtering for CLI status display. | Lane-scoped status stays read-only; quota decision authority remains unchanged. |
