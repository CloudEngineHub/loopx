...
def set_goal_activation_state(
    *,
    ...
    registry_paths = [source_registry]
    if registries_are_distinct:
        registry_paths.append(target_registry)
    with ExitStack() as stack:
        transaction = None
        for path in sorted(registry_paths, key=lambda item: str(item)):
            if _same_path(path, source_registry):
        ...
        if transaction is None:
            raise RuntimeError("Goal source registry transaction was not acquired")
        if registries_are_distinct:
